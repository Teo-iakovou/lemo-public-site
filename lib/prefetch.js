...
export async function prefetchBookingData(days = 14) {
  try {
    ...
    const horizonKoushis = fetch(`/api/availability/horizon?start=${start}&days=${days}&barber=${encodeURIComponent("ΚΟΥΣΙΗΣ")}`, { cache: "no-store" });
    ...
    const nextKoushis = fetch(`/api/availability/horizon?start=${nextStart}&days=${days}&barber=${encodeURIComponent("ΚΟΥΣΙΗΣ")}`, { cache: "no-store" });
    const t0 = Date.now();
    const [svcRes] = await Promise.all([svcPromise]);
    // Fire-and-forget warms
    horizonLemo.catch(()=>{});
    horizonForou.catch(()=>{});
    horizonKoushis.catch(()=>{});
    nextLemo.catch(()=>{});
    nextForou.catch(()=>{});
    nextKoushis.catch(()=>{});
