...
export function buildAppointmentRequestPayload(input = {}) {
  ...
  // Multi-slot booking: when slots[] is supplied, normalize each slot's datetime/barber/bookedFor
  // and attach them. The backend routes to its atomic multi-slot path when slots[] is present.
  // We also seed the top-level appointmentDateTime from slot 1 so the Next proxy's single-field
  // validation still passes for a pure slots[] submit.
  if (Array.isArray(input.slots) && input.slots.length > 0) {
    const tz = toTrimmed(input.timeZone) || ATHENS_TIME_ZONE;
    const normalizedSlots = input.slots.map((s) => {
      const rawSlotDate =
        s.appointmentDateTime ??
        s.dateTime ??
        (s.date && s.time ? `${s.date}T${s.time}` : "");
      const slotDate = normalizeAppointmentDateTime(rawSlotDate, {
        timeZone: tz,
        date: s.date,
        time: s.time,
      });
      const slot = {
        appointmentDateTime: slotDate,
        barber: normalizeBarberValue(s.barber),
      };
      const bf = toTrimmed(s.bookedFor);
      if (bf) slot.bookedFor = bf;
      return slot;
    });
    payload.slots = normalizedSlots;
    if (!payload.appointmentDateTime && normalizedSlots[0]?.appointmentDateTime) {
      payload.appointmentDateTime = normalizedSlots[0].appointmentDateTime;
      payload.dateTime = normalizedSlots[0].appointmentDateTime;
    }
  }
