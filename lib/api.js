...
export async function rescheduleAppointment(id, payload = {}) {
  ...
  const normalized = buildAppointmentRequestPayload({
    appointmentDateTime: payload.appointmentDateTime || payload.dateTime,
    date: payload.date,
    time: payload.time,
  });
  const resolvedDateTime = normalized.appointmentDateTime || payload.appointmentDateTime || payload.dateTime;
  const requestBody = {
    ...payload,
  ...
  const res = await fetch(`/api/appointments/mine/${encodeURIComponent(id)}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body,
  });
  ...
  if (!res.ok) {
    throw new Error(data?.error || text || "Failed to update appointment");
  }
  return data;
}
