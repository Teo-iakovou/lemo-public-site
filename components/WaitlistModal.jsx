...
export default function WaitlistModal({
  ...
  useEffect(() => {
    ...
    getAvailability({
      ...
      .catch((err) => {
        setSlotsError(err?.message || "Αποτυχία φόρτωσης ωρών");
        setSuggestedSlots([]);
      })
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-[#a855f7]/40 bg-[#07080d] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.6)] text-white">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            {readableDate && (
              <p className="text-sm text-white/60">Ημερομηνία: {readableDate}</p>
            )}
          </div>
          <button
            onClick={() => onClose?.()}
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          >
            {showSuggestions ? "Απόκρυψη ωρών" : "Διαθέσιμες ώρες"}
          </button>
          {showSuggestions && (
            <div className="rounded-lg border border-white/10 bg-white/5 p-3">
              {loadingSlots && <p className="text-sm text-white/60">Φόρτωση...</p>}
              {!loadingSlots && slotsError && (
                <p className="text-sm text-red-400">{slotsError}</p>
              )}
              {!loadingSlots && !slotsError && suggestedSlots.length === 0 && (
                <p className="text-sm text-white/60">Όλες οι ώρες είναι διαθέσιμες σήμερα.</p>
              )}
              {!loadingSlots && !slotsError && suggestedSlots.length > 0 && (
